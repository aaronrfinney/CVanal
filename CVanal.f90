...
!===============================================================================
  ...
  logical :: skip
  real(dp) :: tau,tzero,tfinal,avMncl
  real(dp), save :: otime
  integer :: maxclmol
...
!
  ...
  write(un_log,'(/,a)') "***** Distribution of firstshell CC-CC Distances for 6 coordinate Ca"
  do i=1,distbins
     write(un_log,'(f8.3,f11.6)') (i-0.5)*distbindelta,distfscc(i)
  end do

  write(un_log,'(/,a)') "***** Distribution of firstshell CC--CA--CC Angles for 6 coordinate Ca"
  do i=1,angbins
     write(un_log,'(f8.3,f11.6)') (i-0.5)*angbindelta,angfscc(i)
  end do  

  write(un_log,'(/,a)') "***** Distribution of firstshell CA-CC Distances for 6 coordinate Ca"
  do i=1,distbins
...
!====Average Manhatten distance based on average distributions
  ...
  write(un_log,'(a)') "***** Note that the below averages are for 6 coordinate Ca only"
  write(un_log,'(a)') "Atom index, trajectory fraction, Aragonite C-C dist. MD, &
       &Calcite C-C dist. MD, Aragonite C-Ca-C ang. MD, &
       & Calcite C-Ca-C ang. MD, Aragonite Ca-C dist. MD, Calcite Ca-C dist MD"
  
  do j=1, nca
     if(avMD(j,1,1) .gt. 0.0_dp) then !Ca has to be at least coordinated to C
...
!====Average Manhatten distance based on averages of stepwise MD values
  ...
  !C-C Distances
  do j=1, nca
     if(avMD(j,1,1) .gt. 0.0_dp) then !Ca has to be at least coordinated to 6 C

        !Mean values
        !mean = sumx / N
        mandistara = avMD(j,2,1) / avMD(j,1,1)
        mandistcal = avMD(j,3,1) / avMD(j,1,1)

        !Standard deviations
        !sd = (sumx^2 - (sumx*sumx)/N) / (N-1)
        var_distara = (avMD(j,2,2) - (avMD(j,2,1)*avMD(j,2,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
        var_distcal = (avMD(j,3,2) - (avMD(j,3,1)*avMD(j,3,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
  ...
  do j=1, nca
     if(avMD(j,1,1) .gt. 0.0_dp) then !Ca has to be at least coordinated to 6 C
        ...
        !Mean values
        !mean = sumx / N
        manangara = avMD(j,4,1) / avMD(j,1,1)
        manangcal = avMD(j,5,1) / avMD(j,1,1)

        !Standard deviations
        !sd = (sumx^2 - (sumx*sumx)/N) / (N-1)
        var_angara = (avMD(j,4,2) - (avMD(j,4,1)*avMD(j,4,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
        var_angcal = (avMD(j,5,2) - (avMD(j,5,1)*avMD(j,5,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
  ...
  do j=1, nca
     if(avMD(j,1,1) .gt. 0.0_dp) then !Ca has to be at least coordinated to 6 C
        ...
        !Mean values
        !mean = sumx / N
        mandistcaara = avMD(j,6,1) / avMD(j,1,1)
        mandistcacal = avMD(j,7,1) / avMD(j,1,1)

        !Standard deviations
        !sd = (sumx^2 - (sumx*sumx)/N) / (N-1)
        var_distcaara = (avMD(j,6,2) - (avMD(j,6,1)*avMD(j,6,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
        var_distcacal = (avMD(j,7,2) - (avMD(j,7,1)*avMD(j,7,1))/avMD(j,1,1))/(avMD(j,1,1)-1)
